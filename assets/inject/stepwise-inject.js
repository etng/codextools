(() => {
  ...
  function toggleCodexTheme() {
    ...
    setCodexThemeMode(nextTheme)
      ...
      .catch((error) => {
        console.warn("[CodexTools Stepwise] Failed to switch Codex theme", error);
      });
  }
  ...
  function statusLine(settings) {
    if (settings.enabled !== true) return "Stepwise 已关闭，请在 CodexTools 管理工具里开启。";
  ...
  async function openManager() {
    state.settingsStatus = "正在打开 CodexTools 管理工具...";
    renderFloat();
    const payload = await bridgeCall("/manager/open", { source: "codex_plus_menu" });
    if (!isCurrentInstance()) return;
